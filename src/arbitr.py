...
Description: Controller of the ARBitR pipeline. This script controls the
parameters and workflow of ARBitR.
...
parser.add_argument("-v","--version", \
                    ...
                    action="version", \
                    version = "ARBitR v. {}".format(__version__))
...
def getOut():
    ...
    if args.output:
        ...
        outfilename = args.input_bam.split("/")[-1].split(".bam")[0]+".ARBitR"
    else:
        outfilename = args.input_bam.split(".bam")[0]+".ARBitR"
    return outfilename
...
def writeGfa(outfilename, input_contig_lengths, graph):
    ...
    with open(outfilename + ".gfa", "w", encoding = "utf-8") as gfa:
        gfa.write("H\tVN:Z:ARBitR/link_graph\n")
